...
const TicketNFTContract = async() => {
  ...
  console.log('Here: ', TicketNFTContract?.interface);
...
export const writeEvent = async (
  ...
  const connectedAccount = getGlobalState('connectedAccount');
  console.log('here: ', numVipTickets, numSilverTickets, connectedAccount);
...
export const buyTicket = async (
  ...
  const connectedAccount = getGlobalState('connectedAccount');
  const ticket_id = getGlobalState('ticket_id');
  console.log(Number(eventId),category,typeof(Number(ticket_id)));
...
export const getMyEvents = async () => {
  ...
  if (address) {
    try {
      ...
      console.log('myEvents',allMyEvents);
...
export const safeMint = async (ticket_id:number, uri:string) => {
  ...
  if (address) {
    try {
      ...
      const result = await ticketNFTContract?.safeMint(address, ticket_id, uri,{ from: address })
      if (result) {
